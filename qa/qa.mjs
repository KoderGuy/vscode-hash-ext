...
// ---------------------------------------------------------------------------
// The dataset is built INTO this package (see import below), so the harness
// performs no commit checks of its own — qa/run-qa.mjs already guarantees
// only the COMMITTED qa-dist/qa.pkg.mjs is ever executed. The only thing
// resolved from disk is the freshly-built CODE UNDER TEST; `npm run qa` runs
// from the repo root so process.cwd() locates it (no git needed here).
// ---------------------------------------------------------------------------
const repoRoot = process.cwd();

// Code UNDER TEST: the extension's production functions, freshly built each
// run by `npm run build:test` (gitignored). The frozen test harness validates
// the CURRENT extension code — only the harness itself is frozen, not the
// code it tests.
let prod;
try {
  prod = require(path.join(repoRoot, 'qa', 'algorithms.cjs'));
} catch (e) {
  console.error(
